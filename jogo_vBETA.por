     ...
     enquanto(pronto == ""){
     	...
     	se(turno == 1){
     		...
     		especificar = 2
				...
				leia(ok)
					...
					escreva(" Nesse momento nosso(a) heroi(na) se recorda de seus companheiros e retorna rapidamente")
					}
				}
				}
     				enquanto(acao_mun == "1"){
     	...
     	se(turno == 1){
			...
			arma = 1
     		...
     		enquanto(momento == 9){
     			se(especificar != 11){
				escreva(n, "elfa: eu gostaria de agradece-lo por antes, aqui (eu te da um artefato misterioso, em	|   |	(mesmo você achando isso bizarro você obedece ela, colocando toda sua confiança nela\nque o item é semelhante a uma moeda) elfa:agora você deve come-la, vai logo bobinho	|   |	em seguida o ingere com desgosto, incredulo que vai fazer isso com tal objeto")
     			leia(ok)
     			escreva(n, "(Você começa a ficar mal, você não se sente bem, pelo contrário, você se sente pior do	|   |     (Derepente, você começa a se levanta, dessa vez mais\nque nunca, você se sente traído por quem confiou, será o fim da aventura?)		|   |		leve, mais forte, com mais energia e... mágico! Você nunca se sentiu assim)")
     			leia(ok)
